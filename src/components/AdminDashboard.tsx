...
import { 
  ...
  Briefcase, Calendar, Clock, Clipboard, FileText, Phone, Send, MessageSquare, Cloud,
...
import { googleSignIn, googleSignOut, getCachedToken } from '../lib/drive';
import Customer360 from './Customer360';
import EnterpriseDashboard from './EnterpriseDashboard';
import NotificationCenter from './NotificationCenter';
import LeadManagementAndPipeline from './LeadManagementAndPipeline';
import EnterprisePayrollWorkspace from './EnterprisePayrollWorkspace';
import EnterpriseReportsWorkspace from './EnterpriseReportsWorkspace';
...
export default function AdminDashboard({ user, onLogout, onUserUpdate }: AdminDashboardProps) {
  ...
  const [activeTab, setActiveTab] = useState<'analytics' | 'telecallers' | 'upload' | 'leads' | 'recordings' | 'resets' | 'payroll' | 'backups' | 'autocall' | 'support' | 'hrm' | 'reports'>('analytics');

  // Enterprise UI/UX Redesign States
  const [isSidebarCollapsed, setIsSidebarCollapsed] = useState<boolean>(false);
  const [isMobileMenuOpen, setIsMobileMenuOpen] = useState<boolean>(false);
  const [globalSearchQuery, setGlobalSearchQuery] = useState<string>('');
  const [selectedWorkspace, setSelectedWorkspace] = useState<string>('hub-hq');
  const [selectedTenant, setSelectedTenant] = useState<string>('node-primary');
  ...
  const [activeCustomer360Lead, setActiveCustomer360Lead] = useState<any | null>(null);

  useEffect(() => {
    if (activeCustomer360Lead) {
      const updated = leads.find(l => l.id === activeCustomer360Lead.id);
      if (updated) {
        setActiveCustomer360Lead(updated);
      }
    }
  }, [leads]);
  ...
  const [showAuthDomainModal, setShowAuthDomainModal] = useState(false);
  const [copiedDomain, setCopiedDomain] = useState<string | null>(null);
  const [isDriveBackupEnabled, setIsDriveBackupEnabled] = useState(false);
  ...
  const [queryResponsePayType, setQueryResponsePayType] = useState<string>('Full Pay');
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        try {
          const testingRes = await fetch('/api/testing-period', { headers: { 'x-user-role': user.role, 'x-user-id': user.id } });
          if (testingRes.ok) {
        ...
        if (user.role === 'admin' || user.role === 'sub-admin') {
          const gpsAuthRes = await fetch('/api/admin/login-authority-requests', { headers: { 'x-user-role': user.role, 'x-user-id': user.id } });
          if (gpsAuthRes.ok) {
  ...
  const fetchSalaryRules = async () => {
    ...
    try {
      const res = await fetch('/api/admin/salary-rules', { headers: { 'x-user-role': user.role, 'x-user-id': user.id } });
      if (res.ok) {
        const data = await res.json();
        if (data.success) {
    ...
    const fetchCustomData = async () => {
      try {
        const [instRes, commsRes] = await Promise.all([
          fetch('/api/admin/improvement-instructions', { headers: { 'x-user-role': user.role, 'x-user-id': user.id } }),
          fetch('/api/sub-admin/comms', { headers: { 'x-user-role': user.role, 'x-user-id': user.id } })
        ]);
  ...
  // Sync logged-in manager's profile state
  useEffect(() => {
    if (isProfileInitialized) return; // Prevent overriding user's manual inputs / typing state!

    const currentUserData = allUsers.find(u => u.id === user.id);
    if (currentUserData) {
      setProfileName(currentUserData.name);
      setProfileEmail(currentUserData.email);
      setProfilePassword(currentUserData.password || '');
      setProfilePhone(currentUserData.phone || '');
      setProfileWhatsapp(currentUserData.whatsapp || '');
      setIsProfileInitialized(true);
    } else if (user && !isProfileInitialized) {
      setProfileName(user.name);
      setProfileEmail(user.email);
      setProfilePhone(user.phone || '');
      setProfileWhatsapp(user.whatsapp || '');
    }
  }, [allUsers, user, isProfileInitialized]);
  ...
  const fetchDriveBackups = async (token: string) => {
    try {
      ...
      if (res.ok) {
        const d = await res.json();
        if (d.success) {
          setDriveBackups(d.files || []);
          setDriveError(null);
        } else {
          setDriveError(d.error || 'Failed to list drive backups.');
        }
      } else {
        const d = await res.json();
        setDriveError(d.error || 'Failed to list drive backups.');
      ...
      setDriveError(err.message || 'Error listing drive backups.');
    }
  };
  ...
  const handleGoogleConnect = async () => {
    try {
      ...
      if (isUnauthorizedDomain) {
        setShowAuthDomainModal(true);
        showNotification('Authorization error: Domain not authorized in Firebase Console.', 'error');
      } else {
        showNotification('Google connection failed: ' + (err.message || err), 'error');
      }
    }
  };
  ...
  const handleGoogleDisconnect = async () => {
    try {
      ...
      setDriveBackups([]);
      showNotification('Google Account disconnected. You can connect a new drive now!', 'success');
    } catch (err: any) {
      console.error(err);
      showNotification('Disconnection failed.', 'error');
    }
  };
  ...
  const handleGoogleDriveSync = async () => {
    ...
    try {
      ...
      if (res.ok) {
        const d = await res.json();
        showNotification(d.message || 'Work backups and call recordings synced successfully!', 'success');
        triggerRefresh();
        fetchDriveBackups(googleAccessToken);
      } else {
        const d = await res.json();
        showNotification(d.error || 'Google Drive sync failed.', 'error');
      }
    } catch (err: any) {
      console.error(err);
      showNotification('Error syncing with Google Drive: ' + (err.message || err), 'error');
    } finally {
  ...
  // Admin feature: Delete All Call Logs and audio recordings (Main Admin Only)
  const handleDeleteAllCallLogs = () => {
    showConfirm(
      "DELETE ALL CALL RECORDINGS (सभी कॉल रिकॉर्डिंग्स हटाएं)",
      "WARNING: This will permanently delete all recorded call logs and physical audio MP3 files from the server. This action cannot be undone. Are you sure? (चेतावनी: इससे सर्वर से सभी कॉल लॉग और ऑडियो MP3 फ़ाइलें स्थायी रूप से हट जाएंगी।)",
      async () => {
        try {
          const res = await fetch('/api/calls/delete-all', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
              'x-user-role': user.role,
          ...
          if (res.ok) {
            showNotification('All recorded call logs and files deleted successfully!', 'success');
            triggerRefresh();
          } else {
            const errData = await res.json();
            showNotification(errData.error || 'Failed to delete all call logs', 'error');
          }
        } catch (err) {
          showNotification('Network error', 'error');
        }
  ...
  // Admin feature: Export and Save All Call Logs Data as JSON
  const handleSaveAllCallLogsData = () => {
    try {
      const dataStr = "data:text/json;charset=utf-8," + encodeURIComponent(JSON.stringify(callLogs, null, 2));
      const downloadAnchor = document.createElement('a');
      downloadAnchor.setAttribute("href", dataStr);
      downloadAnchor.setAttribute("download", `hubsphere_call_logs_export_${Date.now()}.json`);
      document.body.appendChild(downloadAnchor);
      downloadAnchor.click();
      downloadAnchor.remove();
      showNotification('Call logs data downloaded successfully!', 'success');
    } catch (err) {
      showNotification('Failed to export call logs data.', 'error');
    }
  };
  ...
  if (activeCustomer360Lead) {
    return (
      <Customer360
        lead={activeCustomer360Lead}
        currentUser={user}
        onClose={() => setActiveCustomer360Lead(null)}
  ...
  const renderSidebarContent = (isMobile: boolean) => {
    ...
    return (
      <div className="flex-1 flex flex-col h-full gap-5 text-left">
        {/* Logo and company branding */}
        <div className="flex items-center justify-between pb-4 border-b border-gray-800/80">
          <div className="flex items-center gap-2.5 overflow-hidden text-left">
            {collapsed ? (
              <div className="bg-orange-500/10 border border-orange-500/20 p-2.5 rounded-xl text-orange-500 flex items-center justify-center shrink-0" title={BRAND_CONFIG.VERSION_NAME}>
                <Shield className="w-5 h-5 fill-orange-500/15" />
              </div>
            ) : (
              <div className="flex flex-col gap-1.5">
                <Logo theme="dark" textSizeClassName="text-base" iconClassName="w-4 h-4" />
                <div className="flex items-center gap-1.5 pl-0.5">
                  <VersionBadge className="scale-75 origin-left" />
                  <span className="text-[9px] text-amber-500 font-extrabold tracking-wider uppercase bg-amber-500/10 px-1.5 py-0.5 rounded border border-amber-500/20">
                    ERP HQ
                  </span>
                </div>
              </div>
          ...
          {!isMobile && (
            <button 
              ...
              className="hidden md:flex p-1.5 hover:bg-gray-800 rounded-lg text-gray-400 hover:text-white transition cursor-pointer border border-gray-800"
        ...
        <div className="space-y-1.5">
          {!collapsed && (
            <span className="text-[10px] font-bold text-gray-500 uppercase tracking-widest block mb-1 px-2.5">
              Operational Tasks
            </span>
          )}
          <nav className="flex flex-col gap-1">
            {isTabAllowed('analytics') && (
              <button
                id="tab-analytics"
                onClick={() => { setActiveTab('analytics'); if (isMobile) setIsMobileMenuOpen(false); }}
                className={`relative w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-xs font-semibold transition-all group cursor-pointer ${
                  activeTab === 'analytics'
              ...
              >
                ...
                <BarChart3 className={`w-4 h-4 flex-shrink-0 group-hover:scale-110 transition ${activeTab === 'analytics' ? 'text-white' : 'text-gray-400 group-hover:text-white'}`} />
                {!collapsed && <span>Interactive Analytics</span>}
              </button>
            )}
            ...
            {isTabAllowed('telecallers') && (
              <button
                id="tab-telecallers"
                onClick={() => { setActiveTab('telecallers'); if (isMobile) setIsMobileMenuOpen(false); }}
                className={`relative w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-xs font-semibold transition-all group cursor-pointer ${
              ...
              >
                ...
                <Users className={`w-4 h-4 flex-shrink-0 group-hover:scale-110 transition ${activeTab === 'telecallers' ? 'text-white' : 'text-gray-400 group-hover:text-white'}`} />
                {!collapsed && (
                  <span className="truncate">Staff ({telecallers.length + admins.length})</span>
                )}
              </button>
            )}
            ...
            {isTabAllowed('upload') && (
              <button
                id="tab-upload"
                onClick={() => { setActiveTab('upload'); if (isMobile) setIsMobileMenuOpen(false); }}
                className={`relative w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-xs font-semibold transition-all group cursor-pointer ${
                  activeTab === 'upload'
                    ? 'bg-orange-500 text-white shadow-lg shadow-orange-500/20 font-black'
                    : 'text-gray-400 hover:text-white hover:bg-gray-800/60'
              ...
              >
                ...
                <Upload className={`w-4 h-4 flex-shrink-0 group-hover:scale-110 transition ${activeTab === 'upload' ? 'text-white' : 'text-gray-400 group-hover:text-white'}`} />
                {!collapsed && <span>Leads Upload Center</span>}
              </button>
            )}
            ...
            {isTabAllowed('leads') && (
              <button
                id="tab-leads"
                onClick={() => { setActiveTab('leads'); if (isMobile) setIsMobileMenuOpen(false); }}
                className={`relative w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-xs font-semibold transition-all group cursor-pointer ${
                  activeTab === 'leads'
                    ? 'bg-orange-500 text-white shadow-lg shadow-orange-500/20 font-black'
                    : 'text-gray-400 hover:text-white hover:bg-gray-800/60'
              ...
              >
                ...
                <Database className={`w-4 h-4 flex-shrink-0 group-hover:scale-110 transition ${activeTab === 'leads' ? 'text-white' : 'text-gray-400 group-hover:text-white'}`} />
                {!collapsed && <span className="truncate">Active Leads Database</span>}
              </button>
            )}
            ...
            {isTabAllowed('recordings') && (
              <button
                id="tab-recordings"
                onClick={() => { setActiveTab('recordings'); if (isMobile) setIsMobileMenuOpen(false); }}
                className={`relative w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-xs font-semibold transition-all group cursor-pointer ${
              ...
              >
                ...
                <Disc className={`w-4 h-4 flex-shrink-0 group-hover:scale-110 transition ${activeTab === 'recordings' ? 'text-white' : 'text-gray-400 group-hover:text-white'}`} />
                {!collapsed && <span>Recorded Call Logs</span>}
              </button>
            )}
            ...
            {isTabAllowed('resets') && (
              <button
                id="tab-resets"
                onClick={() => { setActiveTab('resets'); if (isMobile) setIsMobileMenuOpen(false); }}
                className={`relative w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-xs font-semibold transition-all group cursor-pointer ${
                  activeTab === 'resets'
                    ? 'bg-orange-500 text-white shadow-lg shadow-orange-500/20 font-black'
                    : 'text-gray-400 hover:text-white hover:bg-gray-800/60'
              ...
              >
                ...
                <Key className={`w-4 h-4 flex-shrink-0 group-hover:scale-110 transition ${activeTab === 'resets' ? 'text-white' : 'text-gray-400 group-hover:text-white'}`} />
                {!collapsed && <span>Password Resets</span>}
              </button>
            )}
          </nav>
        </div>

        {/* SECTION 2: FINANCE & AUDITING */}
        <div className="space-y-1.5">
          {!collapsed && (
            <span className="text-[10px] font-bold text-gray-500 uppercase tracking-widest block mb-1 px-2.5">
              Finance & Audit
          ...
          <nav className="flex flex-col gap-1">
            {isTabAllowed('payroll') && (
              <button
                id="tab-payroll"
                onClick={() => { setActiveTab('payroll'); if (isMobile) setIsMobileMenuOpen(false); }}
                className={`relative w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-xs font-semibold transition-all group cursor-pointer ${
                  activeTab === 'payroll'
                    ? 'bg-orange-500 text-white shadow-lg shadow-orange-500/20 font-black'
                    : 'text-gray-400 hover:text-white hover:bg-gray-800/60'
              ...
              >
                ...
                <DollarSign className={`w-4 h-4 flex-shrink-0 group-hover:scale-110 transition ${activeTab === 'payroll' ? 'text-white' : 'text-gray-400 group-hover:text-white'}`} />
                {!collapsed && <span>Payroll Integration</span>}
              </button>
            )}
            ...
            {isTabAllowed('hrm') && (
              <button
                id="tab-hrm"
                onClick={() => { setActiveTab('hrm'); if (isMobile) setIsMobileMenuOpen(false); }}
                className={`relative w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-xs font-semibold transition-all group cursor-pointer ${
                  activeTab === 'hrm'
                    ? 'bg-orange-500 text-white shadow-lg shadow-orange-500/20 font-black'
                    : 'text-gray-400 hover:text-white hover:bg-gray-800/60'
              ...
              >
                ...
                <Briefcase className={`w-4 h-4 flex-shrink-0 group-hover:scale-110 transition ${activeTab === 'hrm' ? 'text-white' : 'text-gray-400 group-hover:text-white'}`} />
                {!collapsed && <span>HRM Management</span>}
              </button>
            )}
            ...
            {isTabAllowed('reports') && (
              <button
                id="tab-reports"
                onClick={() => { setActiveTab('reports'); if (isMobile) setIsMobileMenuOpen(false); }}
                className={`relative w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-xs font-semibold transition-all group cursor-pointer ${
                  activeTab === 'reports'
                    ? 'bg-orange-500 text-white shadow-lg shadow-orange-500/20 font-black'
                    : 'text-gray-400 hover:text-white hover:bg-gray-800/60'
              ...
              >
                ...
                <BarChart3 className={`w-4 h-4 flex-shrink-0 group-hover:scale-110 transition ${activeTab === 'reports' ? 'text-white' : 'text-gray-400 group-hover:text-white'}`} />
                {!collapsed && <span>Reports & BI</span>}
              </button>
            )}
            ...
            {isTabAllowed('backups') && (
              <button
                id="tab-backups"
                onClick={() => { setActiveTab('backups'); if (isMobile) setIsMobileMenuOpen(false); }}
                className={`relative w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-xs font-semibold transition-all group cursor-pointer ${
                  activeTab === 'backups'
                    ? 'bg-orange-500 text-white shadow-lg shadow-orange-500/20 font-black'
                    : 'text-gray-400 hover:text-white hover:bg-gray-800/60'
              ...
              >
                ...
                <HardDrive className={`w-4 h-4 flex-shrink-0 group-hover:scale-110 transition ${activeTab === 'backups' ? 'text-white' : 'text-gray-400 group-hover:text-white'}`} />
                {!collapsed && <span>Excel Backups</span>}
              </button>
            )}
            ...
            {isTabAllowed('autocall') && (
              <button
                id="tab-autocall"
                onClick={() => { setActiveTab('autocall'); if (isMobile) setIsMobileMenuOpen(false); }}
                className={`relative w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-xs font-semibold transition-all group cursor-pointer ${
                  activeTab === 'autocall'
                    ? 'bg-orange-500 text-white shadow-lg shadow-orange-500/20 font-black'
                    : 'text-gray-400 hover:text-white hover:bg-gray-800/60'
              ...
              >
                ...
                <Settings className={`w-4 h-4 flex-shrink-0 group-hover:scale-110 transition ${activeTab === 'autocall' ? 'text-white' : 'text-gray-400 group-hover:text-white'}`} />
                {!collapsed && <span>Auto-Calling Setup</span>}
              </button>
            )}
            ...
            {isTabAllowed('support') && (
              <button
                id="tab-support"
                onClick={() => { setActiveTab('support'); if (isMobile) setIsMobileMenuOpen(false); }}
                className={`relative w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-xs font-semibold transition-all group cursor-pointer ${
                  activeTab === 'support'
                    ? 'bg-orange-500 text-white shadow-lg shadow-orange-500/20 font-black'
                    : 'text-gray-400 hover:text-white hover:bg-gray-800/60'
              ...
              >
                ...
                <CheckCircle className={`w-4 h-4 flex-shrink-0 group-hover:scale-110 transition ${activeTab === 'support' ? 'text-white' : 'text-gray-400 group-hover:text-white'}`} />
                {!collapsed && <span>24/7 Tech Support</span>}
              </button>
            )}
          </nav>
        </div>

        {/* Live system monitoring status badge (Desktop only) */}
        {!collapsed && (
          <div className="mt-auto p-3.5 bg-gray-900/40 border border-gray-800 rounded-2xl flex flex-col gap-1 text-left select-none">
            <span className="text-[8px] font-black tracking-widest text-gray-500 uppercase">SYS SECURE SHIELD</span>
            <div className="flex items-center gap-2">
              <span className="w-2 h-2 bg-emerald-500 rounded-full animate-ping"></span>
              <span className="text-xs font-bold text-emerald-400 tracking-wide uppercase">TLS 1.3 Active</span>
            </div>
          </div>
        )}

        {/* User profile section at bottom */}
        <div className={`mt-auto pt-4 border-t border-gray-800 text-left ${collapsed ? 'mt-0' : ''}`}>
          <div className="flex items-center gap-3 overflow-hidden">
            <div className="w-9 h-9 rounded-xl bg-orange-500/15 border border-orange-500/30 text-orange-400 font-extrabold flex items-center justify-center flex-shrink-0 uppercase">
              {user.name ? user.name[0] : 'U'}
            </div>
            {!collapsed && (
              <div className="flex flex-col min-w-0 flex-1">
                <span className="text-xs font-black text-white truncate leading-none">{user.name}</span>
                <span className="text-[9px] text-gray-500 truncate mt-1 capitalize leading-none font-bold">
                  {user.role === 'admin' ? (user.id === 'u-admin' ? 'Main Admin' : 'Admin') : user.role === 'sub-admin' ? 'Sub-Admin' : 'Dept Head'}
                </span>
              </div>
            )}
            
            {/* Minimal Logout Button inside Sidebar */}
            {!collapsed && (
              <button 
                onClick={onLogout}
                className="p-1.5 hover:bg-red-500/10 hover:text-red-400 text-gray-500 rounded-lg transition cursor-pointer"
                title="Secure Logout"
              >
  ...
  const getBreadcrumbTabLabel = (tab: string) => {
    switch (tab) {
      case 'analytics': return 'Interactive Analytics';
      case 'telecallers': return 'Staff & Admins';
      case 'upload': return 'Leads Upload Center';
      case 'leads': return 'Active Leads Database';
      case 'recordings': return 'Recorded Call Logs';
      case 'resets': return 'Password Resets';
      case 'payroll': return 'Payroll Integration';
      case 'hrm': return 'HRM Management';
      case 'backups': return 'Daily Excel Backups';
      case 'autocall': return 'Auto-Calling Setup';
      case 'support': return '24/7 Tech Support';
      default: return 'Console';
    }
  };

  const filteredGlobalSearchLeads = globalSearchQuery.trim() === ''
    ? []
    : leads.filter(l => 
        l.name.toLowerCase().includes(globalSearchQuery.toLowerCase()) || 
        (l.phone && l.phone.includes(globalSearchQuery)) ||
        (l.email && l.email.toLowerCase().includes(globalSearchQuery.toLowerCase())) ||
        (l.id && l.id.toLowerCase().includes(globalSearchQuery.toLowerCase()))
      ).slice(0, 5);

  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 flex flex-col md:flex-row font-sans selection:bg-orange-500 selection:text-white">
      
      {/* MOBILE HEADER FOR PHONE/TABLET VIEW */}
      <header className="md:hidden flex items-center justify-between px-5 py-3 bg-[#111827] border-b border-gray-800 text-white w-full">
        <div className="flex items-center gap-2.5">
          <Logo theme="dark" textSizeClassName="text-base" iconClassName="w-3.5 h-3.5" />
      ...
      {isMobileMenuOpen && (
        <div className="fixed inset-0 z-50 md:hidden flex">
          ...
          {/* Drawer content */}
          <aside className="relative flex-1 max-w-xs w-full bg-[#111827] text-gray-300 flex flex-col h-full border-r border-gray-800 z-10 p-5 gap-6">
            {/* Close button */}
            <button 
              onClick={() => setIsMobileMenuOpen(false)}
              className="absolute top-4 right-4 p-2 text-gray-400 hover:text-white hover:bg-gray-800 rounded-xl transition"
            >
              <X className="w-5 h-5" />
            </button>
      ...
      <aside className={`hidden md:flex flex-col bg-[#111827] text-gray-300 border-r border-gray-800 transition-all duration-300 relative ${isSidebarCollapsed ? 'w-20' : 'w-64'} flex-shrink-0 p-4`}>
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-[#f8fafc]">
        ...
        <header className="bg-white border-b border-slate-100 px-6 py-4 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 shadow-sm z-30">
          
          {/* Left Block: Breadcrumbs and Entity Selectors */}
          <div className="flex flex-col sm:flex-row sm:items-center gap-4 text-left">
            <div>
              ...
              <div className="flex items-center gap-1.5 text-[10px] font-black uppercase text-slate-400 tracking-wider">
                ...
                <ChevronRight className="w-3 h-3 text-slate-300" />
                <span className="text-slate-600 truncate max-w-[120px] sm:max-w-none">{getBreadcrumbTabLabel(activeTab)}</span>
              </div>
              <h2 className="text-lg font-black text-slate-900 tracking-tight mt-0.5 sm:mt-1">
                {getBreadcrumbTabLabel(activeTab)}
              </h2>
            </div>

            {/* Workspace & Tenant Switchers (Future-ready dropdowns) */}
            <div className="hidden sm:flex items-center gap-2">
              {/* Workspace Switcher */}
              <div className="relative">
                <button
                  onClick={() => { setShowWorkspaceDropdown(!showWorkspaceDropdown); setShowTenantDropdown(false); }}
                  className="flex items-center gap-1.5 bg-slate-50 hover:bg-slate-100 border border-slate-200 px-2.5 py-1.5 rounded-lg text-[10px] font-extrabold tracking-wide uppercase text-slate-600 transition"
                >
                  <span>WS: {selectedWorkspace === 'hub-hq' ? 'Primary' : selectedWorkspace === 'hub-corp' ? 'Corp' : 'Logistics'}</span>
                  <ChevronDown className="w-3 h-3 text-slate-400" />
                </button>
                {showWorkspaceDropdown && (
                  <div className="absolute left-0 mt-1.5 w-48 bg-white border border-slate-200 rounded-xl shadow-xl z-50 p-1.5">
                    <button
                      onClick={() => { setSelectedWorkspace('hub-hq'); setShowWorkspaceDropdown(false); }}
                      className={`w-full text-left px-3 py-2 rounded-lg text-xs font-bold transition ${selectedWorkspace === 'hub-hq' ? 'bg-orange-50 text-orange-600' : 'text-slate-600 hover:bg-slate-50'}`}
                    >
                      HubSphere HQ (Primary)
                    </button>
                    <button
                      onClick={() => { setSelectedWorkspace('hub-corp'); setShowWorkspaceDropdown(false); }}
                      className={`w-full text-left px-3 py-2 rounded-lg text-xs font-bold transition ${selectedWorkspace === 'hub-corp' ? 'bg-orange-50 text-orange-600' : 'text-slate-600 hover:bg-slate-50'}`}
                    >
                      HubSphere Enterprise Corp
                    </button>
                    <button
                      onClick={() => { setSelectedWorkspace('hub-logistics'); setShowWorkspaceDropdown(false); }}
                      className={`w-full text-left px-3 py-2 rounded-lg text-xs font-bold transition ${selectedWorkspace === 'hub-logistics' ? 'bg-orange-50 text-orange-600' : 'text-slate-600 hover:bg-slate-50'}`}
                    >
                      HubSphere Logistics Ltd
                    </button>
                  </div>
                )}
              </div>
              ...
              <div className="relative">
                <button
                  onClick={() => { setShowTenantDropdown(!showTenantDropdown); setShowWorkspaceDropdown(false); }}
                  className="flex items-center gap-1.5 bg-slate-50 hover:bg-slate-100 border border-slate-200 px-2.5 py-1.5 rounded-lg text-[10px] font-extrabold tracking-wide uppercase text-slate-600 transition"
                >
                  <span>Tenant: {selectedTenant === 'node-primary' ? 'Node-Alpha' : 'Node-Beta'}</span>
                  <ChevronDown className="w-3 h-3 text-slate-400" />
                </button>
                {showTenantDropdown && (
                  <div className="absolute left-0 mt-1.5 w-44 bg-white border border-slate-200 rounded-xl shadow-xl z-50 p-1.5">
                    <button
                      onClick={() => { setSelectedTenant('node-primary'); setShowTenantDropdown(false); }}
                      className={`w-full text-left px-3 py-2 rounded-lg text-xs font-bold transition ${selectedTenant === 'node-primary' ? 'bg-orange-50 text-orange-600' : 'text-slate-600 hover:bg-slate-50'}`}
                    >
                      Node-Alpha (Dedicated)
                    </button>
                    <button
                      onClick={() => { setSelectedTenant('node-backup'); setShowTenantDropdown(false); }}
                      className={`w-full text-left px-3 py-2 rounded-lg text-xs font-bold transition ${selectedTenant === 'node-backup' ? 'bg-orange-50 text-orange-600' : 'text-slate-600 hover:bg-slate-50'}`}
                    >
                      Node-Beta (Failover)
                    </button>
                  </div>
          ...
          {/* Right Block: Dynamic Search, Quick Actions, Profile Dropdown, Live Clock */}
          <div className="flex flex-wrap items-center gap-3 w-full md:w-auto md:justify-end">
            
            {/* Global Search Bar (with Interactive Leads Lookup Overlay) */}
            <div className="relative flex-1 md:flex-initial min-w-[180px] sm:min-w-[240px]">
              <div className="relative">
                <Search className="w-4 h-4 text-slate-400 absolute left-3.5 top-1/2 -translate-y-1/2" />
                <input
                  type="text"
                  placeholder="Global Search (Client Leads)..."
                  value={globalSearchQuery}
                  onChange={(e) => setGlobalSearchQuery(e.target.value)}
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl pl-9 pr-8 py-2 text-xs font-semibold focus:outline-none focus:border-orange-500 focus:bg-white text-slate-800 placeholder-slate-400 transition"
                />
                {globalSearchQuery && (
                  <button
              ...
              {/* Dynamic Floating Lead Results Card */}
              {globalSearchQuery.trim() !== '' && (
                <div className="absolute top-full mt-2 left-0 right-0 bg-white border border-slate-200/80 rounded-2xl shadow-2xl z-50 p-2.5 max-h-80 overflow-y-auto text-left">
                  <div className="text-[10px] font-black text-slate-400 uppercase tracking-wider mb-2 px-1">
                    Matched Client Leads ({filteredGlobalSearchLeads.length})
                  </div>
                  {filteredGlobalSearchLeads.map((item) => (
                    <button
                      key={item.id}
                      onClick={() => { setActiveCustomer360Lead(item); setGlobalSearchQuery(''); }}
                      className="w-full flex items-center justify-between p-2 hover:bg-slate-50 rounded-xl text-left transition text-xs border border-transparent hover:border-slate-100"
                    >
                      <div>
                        <div className="font-extrabold text-slate-800">{item.name}</div>
                        <div className="text-[10px] text-slate-500 font-mono mt-0.5">{item.phone} {item.email ? `• ${item.email}` : ''}</div>
                      </div>
                      <span className={`text-[9px] font-extrabold px-2 py-0.5 rounded ${
                        item.status === 'Closed Won' ? 'bg-emerald-500/10 text-emerald-600' :
                        item.status === 'Interested' ? 'bg-amber-500/10 text-amber-600' : 'bg-slate-100 text-slate-600'
                      }`}>
                        {item.status}
                      </span>
                    </button>
                  ))}
                  {filteredGlobalSearchLeads.length === 0 && (
                    <div className="text-center py-6 text-xs text-slate-400 italic">
                      No matching lead files located.
                    </div>
                  )}
                </div>
              )}
            ...
            {/* Quick Actions Dropdown ("+" Icon) */}
            <div className="relative">
              <button
                onClick={() => setShowQuickActionsDropdown(!showQuickActionsDropdown)}
                className="p-2 bg-orange-500 hover:bg-orange-600 text-white rounded-xl shadow-md shadow-orange-500/10 transition cursor-pointer flex items-center justify-center"
                title="Create New Records"
              >
                <Plus className="w-5 h-5" />
              </button>
              {showQuickActionsDropdown && (
                <div className="absolute right-0 mt-2 w-52 bg-white border border-slate-200 rounded-2xl shadow-2xl z-50 p-2 text-left">
                  <div className="text-[9px] font-black text-slate-400 uppercase tracking-widest px-3 py-1.5">
                    Instant Workspace Actions
                  </div>
                  <button
                    onClick={() => { setActiveTab('leads'); setShowQuickActionsDropdown(false); }}
                    className="w-full text-left px-3 py-2 rounded-xl text-xs font-bold text-slate-700 hover:bg-slate-50 hover:text-orange-500 transition flex items-center gap-2"
                  >
                    <span>👤</span> Add Client Prospect
                  </button>
                  <button
                    onClick={() => { setActiveTab('telecallers'); setShowQuickActionsDropdown(false); }}
                    className="w-full text-left px-3 py-2 rounded-xl text-xs font-bold text-slate-700 hover:bg-slate-50 hover:text-orange-500 transition flex items-center gap-2"
                  >
                    <span>👥</span> Add Team Operator
                  </button>
                  <button
                    onClick={() => { setActiveTab('payroll'); setShowQuickActionsDropdown(false); }}
                    className="w-full text-left px-3 py-2 rounded-xl text-xs font-bold text-slate-700 hover:bg-slate-50 hover:text-orange-500 transition flex items-center gap-2"
                  >
                    <span>₹</span> Process Salaries
                  </button>
                  <button
                    onClick={() => { triggerRefresh(); showNotification('Database synchronization successfully requested!'); setShowQuickActionsDropdown(false); }}
                    className="w-full text-left px-3 py-2 rounded-xl text-xs font-bold text-slate-700 hover:bg-slate-50 hover:text-orange-500 transition flex items-center gap-2 border-t border-slate-100 mt-1 pt-2"
                  >
                    <span>🔄</span> Live Force Sync
                  </button>
                </div>
              )}
            </div>

            {/* Existing Database Live-Sync Indicator & Refresh Trigger */}
            <div className="hidden sm:flex items-center gap-2">
              <button 
                onClick={triggerRefresh}
                className="p-2 bg-slate-50 hover:bg-slate-100 border border-slate-200 rounded-xl text-slate-500 hover:text-slate-800 transition cursor-pointer"
            ...
            <div className="relative">
              <button
                ...
                className="flex items-center gap-2 bg-slate-50 hover:bg-slate-100 border border-slate-200 px-3 py-1.5 rounded-xl text-xs font-extrabold text-slate-700 transition cursor-pointer"
              >
                <div className="w-5.5 h-5.5 rounded-lg bg-orange-500 text-white font-extrabold flex items-center justify-center text-[10px] uppercase">
                  {user.name ? user.name[0] : 'U'}
                </div>
              ...
              {showProfileDropdown && (
                <div className="absolute right-0 mt-2 w-64 bg-white border border-slate-200 rounded-2xl shadow-2xl z-50 p-3.5 text-left space-y-3">
                  <div>
                    <h4 className="text-xs font-black text-slate-900">{user.name}</h4>
                    <p className="text-[10px] text-slate-400 font-medium truncate">{user.email}</p>
                  </div>
                  
                  <div className="border-t border-slate-100 pt-2.5 text-[10px] space-y-1 text-slate-500 font-bold">
                    <div>ROLE: <span className="text-orange-500 uppercase">{user.role}</span></div>
                    {user.department && <div>DEPT: <span className="text-slate-800 uppercase">{user.department}</span></div>}
                    {user.position && <div>POST: <span className="text-slate-800">{user.position}</span></div>}
                  </div>

                  <div className="border-t border-slate-100 pt-2.5">
                    <button
                      onClick={() => { onLogout(); setShowProfileDropdown(false); }}
                      className="w-full flex items-center justify-center gap-2 bg-red-50 hover:bg-red-100 text-red-600 px-4 py-2 rounded-xl text-xs font-extrabold transition cursor-pointer"
                    >
                      <LogOut className="w-4 h-4" />
                      Secure Logout
                    </button>
            ...
            {/* Live UTC Clock & Date (Hidden on small mobile screens) */}
            <div className="hidden lg:flex items-center gap-1.5 bg-slate-100 border border-slate-200/60 px-3 py-1.5 rounded-xl text-[10px] font-black text-slate-600 uppercase select-none">
              <CalendarDays className="w-3.5 h-3.5 text-slate-400" />
              <span>
                {currentTime.toLocaleDateString(undefined, { weekday: 'short', month: 'short', day: 'numeric' })}
              </span>
              <span className="text-slate-300">•</span>
              <span className="text-orange-500 font-mono tracking-wider">
        ...
        <main className="flex-1 p-6 overflow-y-auto">
          ...
          {activeTab === 'analytics' && (
            <EnterpriseDashboard
              ...
              setActiveCustomer360Lead={setActiveCustomer360Lead}
          ...
          {/* TAB 4: ACTIVE LEADS DATABASE & KANBAN PIPELINE */}
          {activeTab === 'leads' && (
            <LeadManagementAndPipeline 
              leads={leads}
              onLeadsUpdated={() => setRefreshTrigger(prev => prev + 1)}
            ...
            <div className="space-y-6">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                {user.id === 'u-admin' && (
                  <div className="flex flex-wrap items-center gap-3">
                    <button
                      onClick={handleSaveAllCallLogsData}
                      className="bg-emerald-600 hover:bg-emerald-700 text-white font-extrabold text-xs px-4 py-2.5 rounded-xl transition flex items-center gap-2 cursor-pointer shadow-md shadow-emerald-500/10 hover:scale-[1.02]"
                    >
                      📥 SAVE / EXPORT ALL DATA
                    </button>
                    <button
                      onClick={handleDeleteAllCallLogs}
                      className="bg-red-600 hover:bg-red-700 text-white font-extrabold text-xs px-4 py-2.5 rounded-xl transition flex items-center gap-2 cursor-pointer shadow-md shadow-red-500/10 hover:scale-[1.02]"
                    >
                      🗑️ DELETE ALL CALL RECORDS
                    </button>
                  </div>
                )}
              </div>
          ...
          {activeTab === 'resets' && (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                {/* FORM 2: UPDATE PROFILE */}
                <div className="bg-[#111622] border border-[#1f2635] p-6 rounded-2xl space-y-4">
                  <h3 className="text-sm font-bold text-white uppercase tracking-wider flex items-center gap-2">
                    <Shield className="w-4 h-4 text-[#f97316]" /> {user.id === 'u-admin' ? 'Update Main Admin Profile (मुख्य एडमिन प्रोफाइल)' : 'Update Your Profile (प्रोफ़ाइल अपडेट करें)'}
                  </h3>
                  
                  <form onSubmit={handleUpdateAdminProfile} className="space-y-4">
                    <div>
                      <label className="text-[10px] font-bold text-gray-400 block mb-1">{user.id === 'u-admin' ? 'ADMIN NAME (एडमिन का नाम)' : 'YOUR NAME (आपका नाम)'}</label>
                      <input 
                        type="text" 
                        required
                        value={profileName}
                    ...
                    <div>
                      <label className="text-[10px] font-bold text-gray-400 block mb-1">{user.id === 'u-admin' ? 'ADMIN EMAIL (ईमेल पता)' : 'EMAIL ADDRESS (ईमेल पता)'}</label>
                      <input 
                        type="email" 
                        required
                        value={profileEmail}
                    ...
                    <button 
                      ...
                      className="w-full bg-[#f97316] hover:bg-orange-600 text-white font-bold py-2.5 rounded-xl text-xs transition cursor-pointer"
                    >
                      {user.id === 'u-admin' ? 'Save Admin Profile' : 'Save Profile'}
                    </button>
                  </form>
                </div>
...
{activeTab === 'reports' && (
  ...
  />
                                        ...
                                        <div className="flex gap-1">
                                          ...
                                          <button
                                            onClick={() => {
                                              ...
                                              handleApproveLeave(leave.id, 'Approved', undefined, sel?.value || 'Full Pay');
                                            }}
                    ...
                    {(() => {
                      ...
                      return (
                        <div className="bg-white text-gray-900 rounded-2xl p-6 shadow-2xl border-2 border-gray-200 space-y-6 font-sans printable-slip">
                          {/* Slip Header */}
                          <div className="text-center border-b pb-4 border-gray-200 flex flex-col items-center justify-center gap-1">
                            <Logo theme="light" textSizeClassName="text-lg" withIcon={false} />
                            <p className="text-[11px] text-gray-700 font-bold mt-0.5 font-sans">Salary Slip for {selectedPayrollMonth} • {BRAND_CONFIG.VERSION_NAME}</p>
                          </div>
                          ...
                          <div className="text-center pt-2 text-[10px] text-gray-500 leading-normal border-t border-gray-100 font-sans no-print">
                            {BRAND_CONFIG.VERSION_NAME} Systems CRM • 100% Digital Audited Ledger
                            <button
                              type="button"
                  ...
                  <div className="bg-[#111622] border border-[#1f2635] p-5 rounded-2xl space-y-4">
                    <div>
                      <h3 className="text-sm font-bold text-white flex items-center gap-1.5 uppercase tracking-wide">
                        ...
                        Choose Backup Mode (बैकअप का तरीका चुनें)
                      </h3>
                      <p className="text-[10px] text-gray-400 mt-1 leading-relaxed">
                        Select how you want to preserve your records and voice call databases.
                      </p>
                    </div>

                    {/* Segmented control for switching mode */}
                    <div className="grid grid-cols-2 gap-2 bg-[#0c0f16] p-1 rounded-xl border border-[#1f2635]">
                      <button
                        type="button"
                        onClick={() => setIsDriveBackupEnabled(false)}
                      ...
                      >
                        ...
                        Local Backup (आसान और सुरक्षित)
                      </button>
                      <button
                        type="button"
                        onClick={() => setIsDriveBackupEnabled(true)}
                        className={`py-2 rounded-lg text-xs font-bold transition-all cursor-pointer flex items-center justify-center gap-1.5 ${
                          isDriveBackupEnabled
                            ? 'bg-emerald-600 text-white shadow'
                            : 'text-gray-400 hover:text-white'
                      ...
                      >
                        ...
                        Google Drive Sync (क्लाउड सिंक)
                      </button>
                    </div>

                    {/* Content based on selected mode */}
                    {!isDriveBackupEnabled ? (
                      <div className="bg-[#151922] p-4 rounded-xl border border-[#1f2635]/80 space-y-3.5 text-left">
                        <div className="text-xs text-gray-300 space-y-1.5">
                          <p className="font-bold text-white flex items-center gap-1.5">
                            <span className="w-2 h-2 bg-[#f97316] rounded-full"></span>
                            Offline Local Backup Mode Active (लोकल बैकअप सक्रिय है)
                          </p>
                          <p className="text-[10px] text-gray-400 leading-relaxed">
                            No complex Google account setup or permissions required! You can download the complete active database registry anytime to your desktop with one single click.
                          </p>
                          <p className="text-[10px] text-gray-400 leading-relaxed">
                            गूगल अकाउंट सेटअप या परमिशन की कोई आवश्यकता नहीं है! आप एक क्लिक में पूरा डेटा अपने कंप्यूटर पर सुरक्षित डाउनलोड कर सकते हैं।
                          </p>
                        </div>
                        <a
                          href="/api/backups/download"
                          className="w-full bg-[#f97316] hover:bg-orange-600 text-white font-extrabold text-xs py-2.5 rounded-xl transition cursor-pointer text-center block shadow-md shadow-orange-500/10"
                        >
                          ⬇️ Instant 1-Click Backup (तुरंत डेटा डाउनलोड करें)
                        </a>
                      </div>
                    ) : (
                      <div className="space-y-4">
                        <div className="bg-emerald-500/5 border border-emerald-500/10 p-3 rounded-xl text-[10px] text-emerald-300/90 leading-relaxed">
                          📌 Cloud Sync connects to your personal Google Drive to safely upload all dynamic spreadsheets and audio recordings.
                        </div>
                        ...
                        {!googleUser ? (
                          <div className="space-y-2">
                            <button
                              ...
                              className="w-full bg-[#f97316] hover:bg-orange-600 text-white border border-orange-500 font-extrabold text-xs py-2.5 rounded-xl transition cursor-pointer flex items-center justify-center gap-2 shadow-lg shadow-orange-500/10"
                            >
                            ...
                            <button
                              type="button"
                              onClick={() => setShowAuthDomainModal(true)}
                              className="w-full bg-white/5 hover:bg-white/10 text-gray-400 hover:text-white border border-[#1f2635] font-bold text-[10px] py-1.5 rounded-xl transition cursor-pointer flex items-center justify-center gap-1.5"
                            >
                              🔑 Setup Authorized Domain (डोमेन अधिकृत करें)
                            </button>
                          </div>
                        ) : (
                          <div className="space-y-3">
                            <div className="bg-[#0c0f16] p-3 rounded-xl border border-[#1f2635] flex items-center justify-between gap-2">
                              <div className="overflow-hidden text-left">
                                <span className="text-[10px] font-bold text-emerald-400 block uppercase leading-none mb-1">CONNECTED DRIVE</span>
                                <span className="text-xs font-black text-white block truncate">{googleUser.displayName || googleUser.email}</span>
                                <span className="text-[9px] text-gray-400 block truncate">{googleUser.email}</span>
                              </div>
                              <button
                                type="button"
                                onClick={handleGoogleDisconnect}
                                className="text-[10px] font-bold text-red-400 hover:underline px-2 py-1 bg-red-500/10 hover:bg-red-500/20 rounded-lg transition shrink-0 cursor-pointer"
                              >
                                Disconnect
                              </button>
                            </div>
                            ...
                            <button
                              ...
                              className="w-full bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50 text-white font-extrabold text-xs py-2.5 rounded-xl transition cursor-pointer flex items-center justify-center gap-1.5 uppercase shadow-md shadow-emerald-500/15"
                            >
                              {isSyncingDrive ? (
                                <>
                                  <div className="w-3.5 h-3.5 border-2 border-white border-t-transparent rounded-full animate-spin"></div>
                                  <span>Syncing Work & Recordings...</span>
                                </>
                              ) : (
                                <>
                                  <RefreshCw className="w-3.5 h-3.5 animate-spin-slow" />
                                  <span>☁️ Sync Database & Recordings</span>
                                </>
                              )}
                            </button>

                            {/* Drive Backups List */}
                            <div className="pt-2 border-t border-[#1f2635] text-left">
                              <span className="text-[10px] font-bold text-gray-400 block uppercase mb-1.5">Files on Google Drive</span>
                              {driveError ? (
                                <div className="text-[10px] text-red-400 font-bold">{driveError}</div>
                              ) : driveBackups.length === 0 ? (
                                <div className="text-[10px] text-gray-500 italic text-center py-2">No backups found in Google Drive. Click 'Sync' to save!</div>
                              ) : (
                                <div className="max-h-[140px] overflow-y-auto space-y-1.5 pr-1 divide-y divide-[#1f2635]/50">
                                  {driveBackups.map((file: any) => (
                                    <div key={file.id} className="pt-1.5 flex justify-between items-center text-[10px] text-gray-300">
                                      <div className="truncate pr-2">
                                        <span className="font-bold text-white block truncate" title={file.name}>{file.name}</span>
                                        <span className="text-gray-500 text-[8px]">{new Date(file.createdTime).toLocaleString()}</span>
                                      </div>
            ...
            <div className="p-8 space-y-6 overflow-y-auto max-h-[70vh] printable-slip">
              ...
              <div className="border-b border-[#1f2635] pb-6 flex justify-between items-start">
                <div className="text-left flex flex-col gap-1">
                  <Logo theme="dark" textSizeClassName="text-2xl" withIcon={false} />
                  <p className="text-xs text-gray-500 mt-1 text-left font-bold">Salary Slip for {selectedPayrollMonth} • {BRAND_CONFIG.VERSION_NAME}</p>
                </div>
                <div className="text-right">
      ...
      {/* FIREBASE UNAUTHORIZED DOMAIN TROUBLESHOOTING GUIDE MODAL */}
      {showAuthDomainModal && (
          ...
          <div className="bg-[#111622] border border-[#1f2635] rounded-3xl w-full max-w-lg overflow-hidden shadow-2xl flex flex-col">
            <div className="p-6 border-b border-[#1f2635] bg-[#0d1017] flex justify-between items-center">
              <div className="text-left flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-base font-black text-white">Authorize Google Login & Backup</h3>
                  <p className="text-xs text-gray-400 mt-1">Quick setup to enable Google services for your app</p>
                </div>
              </div>
              <button 
                type="button"
                onClick={() => setShowAuthDomainModal(false)}
                className="text-white hover:bg-orange-600 font-bold text-xs bg-orange-500 px-3 py-2 rounded-lg cursor-pointer transition flex items-center justify-center"
              >
                ✕
              </button>
            </div>

            <div className="p-6 overflow-y-auto max-h-[70vh] space-y-5 text-left">
              {/* Warning Alert - Yellow text made black inside a bright clear banner */}
              <div className="p-4 bg-amber-400 border border-amber-500 rounded-2xl text-xs text-black leading-relaxed space-y-1.5">
                <p className="font-extrabold text-black flex items-center gap-1.5">
                  ⚠️ Action Required (जरूरी सूचना)
                </p>
                <p className="font-medium text-black">
                  आपके ऐप की सुरक्षा के लिए, गूगल साइन-इन केवल अधिकृत (Authorized) डोमेन पर ही काम करता है। चूंकि आपका ऐप एक सुरक्षित प्रीव्यू सर्वर पर चल रहा है, इसलिए आपको नीचे दिए गए डोमेन को फायरबेस में जोड़ना होगा।
                </p>
              </div>

              <div className="space-y-3">
                <label className="text-xs font-black text-gray-400 uppercase tracking-wider">Domains to Copy (कॉपी करने के लिए डोमेन):</label>
                <div className="space-y-2.5">
                  <div className="bg-white p-3 rounded-xl border border-gray-200 flex items-center justify-between gap-3">
                    <div className="overflow-hidden">
                      <span className="text-[9px] font-black text-emerald-600 block uppercase mb-0.5 leading-none">CURRENT ACTIVE DOMAIN</span>
                      <code className="text-xs font-mono text-black font-bold block select-all truncate">{typeof window !== 'undefined' ? window.location.hostname : ''}</code>
                    </div>
                    <button
                      type="button"
                      onClick={() => handleCopyDomain(typeof window !== 'undefined' ? window.location.hostname : '')}
                      className="text-[10px] font-black bg-orange-500 text-white hover:bg-orange-600 px-3 py-1.5 rounded-lg transition shrink-0 shadow-md cursor-pointer"
                    >
                      {copiedDomain === (typeof window !== 'undefined' ? window.location.hostname : '') ? 'Copied! ✅' : 'Copy'}
                    </button>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-2 text-xs">
                <span className="text-xs font-black text-gray-400 uppercase tracking-wider block">सरल चरण-दर-चरण निर्देश (Step-By-Step Guide):</span>
                <ol className="list-decimal pl-4 space-y-2 text-gray-300">
                  <li>नीचे दिए गए नारंगी रंग के <strong>Open Firebase Console ↗</strong> बटन पर क्लिक करें।</li>
                  <li>फायरबेस कंसोल में, बाएं (left) साइडबार से <strong>Authentication</strong> पर क्लिक करें, फिर ऊपर से <strong>Settings</strong> टैब चुनें।</li>
                  <li>सूची में से <strong>Authorized domains</strong> पर क्लिक करें।</li>
                  <li><strong>Add domain</strong> बटन पर क्लिक करें, ऊपर कॉपी किया गया डोमेन पेस्ट करें, और <strong>Add</strong> पर क्लिक करें।</li>
                </ol>
              </div>
            </div>

            <div className="p-4 bg-[#0d1017] border-t border-[#1f2635] flex justify-between items-center gap-4">
              <a
                href="https://console.firebase.google.com/"
                target="_blank"
                rel="noreferrer"
                className="px-4 py-2 bg-orange-500 hover:bg-orange-600 text-white font-bold rounded-xl text-xs transition cursor-pointer flex items-center gap-1.5 shadow-md"
              >
              ...
              <button
                type="button"
                onClick={() => setShowAuthDomainModal(false)}
                className="px-5 py-2 bg-orange-500 hover:bg-orange-600 text-white font-extrabold rounded-xl text-xs transition cursor-pointer uppercase tracking-wider shadow-lg shadow-orange-500/20"
              >
                Okay, Added it!
              </button>
            </div>
